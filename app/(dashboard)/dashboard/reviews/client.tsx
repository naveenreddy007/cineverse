...
import { PencilIcon, Trash2, StarIcon, ThumbsUp, MessageSquare, SortAsc, ImageOff } from "lucide-react"
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu"
import { useToast } from "@/components/ui/use-toast"
import { mockReviews, mockDrafts } from "@/lib/mock-data"
...
export function ReviewsClient({ initialReviews = [], initialDrafts = [], error }: ReviewsClientProps) {
  // FIX 1: Initialize state with props directly
  const [isLoading, setIsLoading] = useState(true)
  const [reviews, setReviews] = useState<Review[]>(initialReviews.length > 0 ? initialReviews : mockReviews)
  const [drafts, setDrafts] = useState<Draft[]>(initialDrafts.length > 0 ? initialDrafts : mockDrafts)
  const [sortOrder, setSortOrder] = useState<"newest" | "oldest" | "highest" | "lowest">("newest")
  const { toast } = useToast()
  const [imageErrors, setImageErrors] = useState<Record<string, boolean>>({})

  // FIX 2: Simplified loading effect with empty dependency array
  useEffect(() => {
  ...
  // FIX 3: Use useMemo for sorted reviews instead of useEffect
                ...
                <Card className="bg-black/40 backdrop-blur-sm border-border/50">
                  <CardHeader className="flex flex-row items-start gap-4 space-y-0">
                    {imageErrors[review.id] ? (
                      <div className="h-24 w-16 flex items-center justify-center bg-black/60 rounded-md">
